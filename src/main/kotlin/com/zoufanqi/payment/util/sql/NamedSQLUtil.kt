...
object NamedSQLUtil {
    ...
    fun getSelectSQL(tableName: String, fieldInfo: String, conditionMap: Map<String, Any?>? = null) = getSelectSQLCommon(tableName, fieldInfo, conditionMap)

    fun getSelectSQL(cls: KClass<*>, conditionMap: Map<String, Any?>? = null) = getSelectSQLCommon(getTableName(cls), getDBField(cls), conditionMap)

    fun getSelectSQL(tableNameCls: KClass<*>, fieldInfo: String, conditionMap: Map<String, Any?>? = null) = getSelectSQLCommon(getTableName(tableNameCls), fieldInfo, conditionMap)
    ...
    fun getAndCondition(conditionMap: Map<String, Any?>): String {
        ...
        conditionMap.forEach {
            if (it.value is List<*>) {
                sqlCondition.append(" AND ").append(it.key).append(" IN (:").append(it.key).append(")")
            } else {
                sqlCondition.append(" AND ").append(it.key).append("=:").append(it.key)
            }
        }
    ...
    fun getTableName(cls: KClass<*>) = fieldToDBField(cls.simpleName!!)
